...
class Account:
    ...
    def deposit(self, amount):
        if amount > 0:
            self.balance += amount
            st.write(f"Deposited {amount}. New balance is {self.balance}.")
        else:
            st.write("Deposit amount must be positive.")
    ...
    def withdraw(self, amount):
        if amount > self.balance:
            ...
            self.balance -= amount
            st.write(f"Withdrew {amount}. New balance is {self.balance}.")
        else:
            st.write("Withdrawal amount must be positive.")
...
class SavingsAccount(Account):
    ...
    def calculate_interest(self):
        ...
        st.write(f"Interest calculated: {interest}")
        self.balance += interest
        st.write(f"New balance after interest: {self.balance}")
...
class Transaction:
    ...
    def execute(self):
        try:
            ...
            st.write(f"Transaction {self.transaction_id} executed successfully.")
        except InsufficientFundsError as e:
            st.write(f"Transaction {self.transaction_id} failed: {e}")
...
def main():
    ...
    if choice == "Create Account":
        ...
        initial_balance = st.number_input("Enter initial balance", min_value=0.0)

        if account_type == "savings":
            interest_rate = st.number_input("Enter interest rate (e.g., 0.01 for 1%)", min_value=0.0)
